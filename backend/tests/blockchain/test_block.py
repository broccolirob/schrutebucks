...
def test_mine_block():
    ...
    assert hex_to_binary(block.block_hash)[
...
def test_is_valid_block_jumped_difficulty(last_block, block):
    ...
    block.block_hash = f'{"0" * jumped_difficulty}111abc'
...
def test_is_valid_block_bad_block_hash(last_block, block):
    block.block_hash = '0000000000000000bbbabc'

    with pytest.raises(Exception, match="block_hash must be correct"):
        Block.is_valid_block(last_block, block)
